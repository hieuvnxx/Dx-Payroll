...
class PayrollController extends BaseController
{
    ...
    protected $repoPayrollSettings, $redisConfigForm, $repoEmployee, $zoho, $records;

    public function __construct(PayrollSettingsInterface $repoPayrollSetings, RedisConfigFormInterface $redisConfigForm,
        RedisConfigFormRepository $redisControl, EmployeeInterface $repoEmployee, ZohoController $zohoController, RecordsRepository $records)
    {
    ...
    public function offerDate($inputData = [], $fromDay = '', $toDay = '')
    {
        $response = [];
        if(!empty($inputData)){
            foreach ($inputData as $key => $offer){
                if(empty($offer['From_Date']) || $offer['From_Date'] == '0000-00-00' || strtotime($offer['From_Date']) > strtotime($toDay)  || (!empty($offer['To_Date']) && strtotime($offer['To_Date']) < strtotime($fromDay)) ){
                    continue;
                }else{
                    if(!isset($response['from_offer'])){
                        $response['from_offer'] = date('d-m-Y', strtotime($offer['From_Date']));
                    }
                    $endOffer = date('d-m-Y', strtotime($offer['To_Date']));
                    if(empty($offer['To_Date'])){
                        $endOffer = date('d-m-Y', strtotime($toDay));
                    }
                    $response['to_offer'] = $endOffer;
                }
            }
        }
        return $response;
