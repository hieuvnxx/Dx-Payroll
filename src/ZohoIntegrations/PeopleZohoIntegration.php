...
class PeopleZohoIntegration
{
    public static $zohoToken;

    public function __construct()
    {
        self::$zohoToken = BaseZohoIntegration::getInstance();
        self::$zohoToken->setPeopleUrl('https://people.zoho.com/api/');
        dd(1);
    }
    ...
    static public function callZoho(string $action = '', array $parameter = [], bool $convert = true, string $method = 'POST'): array
    {
        ...
        try {
            ...
            $url = self::$zohoToken->getPeopleUrl() .$action;
    ...
    static public function randomToken(){
        ...
        if(!empty($item)){
            ...
            if(isset($randomToken['last_time']) && $randomToken['last_time'] != '' &&
                ...
                $response = self::getListRefreshToken($randomToken['refresh_token']);
                $item[$updateIndex] = $response[0];
            }
            $result = $item[$updateIndex];
        }else {
            $item = self::getListRefreshToken();
            if(sizeof($item) > 0){
        ...
        if($items->count() > 0) {
            ...
            foreach ($items as $item) {
                ...
                if($diffSecond >= 3500) {
                    ...
                    $response = self::refreshToken($body);
